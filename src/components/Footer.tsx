    ...
    <footer className="w-full border-t border-white/5 bg-slate-950">
      <div className="max-w-7xl mx-auto px-6 sm:px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-8">
        <div className="text-center md:text-left">
          <div className="text-blue-300 font-bold tracking-tighter">
            <span className="text-blue-200">{`<`}</span>KET
        ...
        <nav aria-label="Footer links" className="flex gap-10">
          {links.map((l) => (
            <a
              ...
              className="font-mono text-[10px] tracking-widest uppercase text-slate-500 hover:text-blue-200 transition-colors opacity-80 hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-primary/40 rounded"
            >
              {l.name}
            </a>
          ))}
